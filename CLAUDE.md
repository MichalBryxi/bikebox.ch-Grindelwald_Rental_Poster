...
src/assets/generated/*  Machine-derived assets (currently just
                        hero-mountains-mask.png). Gitignored — never
                        edited by hand, regenerated by bin/*.sh via
                        predev/prebuild. If it's under src/assets/generated/,
                        it was produced from something under
                        src/assets/images/ and can be reproduced by
                        re-running the script.
bin/generate-mask.sh    Produces src/assets/generated/hero-mountains-mask.png.
...
The generated sky mask (`src/assets/generated/`) lives inside `src/` rather
than in `dist/`, even though it's build output in spirit: the dev server
only serves `src/` (nothing outside it resolves via relative `url()`), and
`vite build` empties `dist/` at the start of every build — before this
script's output would ever get inlined. Sitting under `src/` keeps it
visible to both dev and build while still being gitignored like any other
generated artifact.
...
- **Hero photo** (`hero-02.jpg`) — the active header background. Downsized
  ...
  Whichever photo is live, `bin/generate-mask.sh` must point at the same
  file — see its `SRC` variable.
