...
dist/                   Build output. Gitignored — regenerated from
                        src/, never edited by hand, never committed.
```

`src/index.html`, `src/styles.css`, and `src/assets/` all live under `src/`
at the same level — no `../` between them anywhere. That's deliberate:
Vite's dev server resolves an HTML/CSS relative URL like `../assets/x.png`
against the *served page's URL path* (which collapses `../` at the root and
404s), not against the source file's real location on disk — even though
the production build resolves the same path correctly via real filesystem
resolution. Dev and build disagreeing on a path that escapes the root is
exactly the bug that bit us once; keeping everything sibling-level under
`src/` avoids the mismatch entirely rather than working around it.

**Edit `src/index.html` / `src/styles.css`, never `dist/index.html`
directly** — the dist file is regenerated from scratch on every build and
any hand edits to it will be silently lost.
...
save to `src/index.html`, `src/styles.css`, or any file under
`src/assets/`. Run `npm run build` before printing or handing off
`dist/index.html`, so it's never stale relative to the source.
...
qr.save('src/assets/images/qr-code.svg', scale=10, border=1, dark='#1b1f1d', light='#ffffff')
"
```
...
`qr-code.svg` currently in `src/assets/images/` already has the fix applied).
...
- **Hero photo** (`hero-02.jpg`) — the active header background. Downsized
  from a 6720×4480 (30MP, 8.3MB) original to 2400px wide (~1.8MB) — full
  resolution was massively overkill for a background image and bloated the
  build to 18.6MB. `hero-bachalpsee.jpg` (Bachalpsee, public domain, via
  Wikimedia Commons — `Bachalpsee_summer.jpg` by Johnw) is the *previous*
  hero photo, kept in `src/assets/images/` but no longer referenced anywhere.
- **Bike cutouts** (`bike-ghost-e-teru.png`, `bike-moustache-game5.png`) —
  ...
  - **Currently not shown:** the `<div class="hero-bikes">` markup in
    `index.html` is HTML-commented out (bikes removed from the header on
    request) rather than deleted — uncomment it to bring them back.
